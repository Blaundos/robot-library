...
class TestProjectLibrary:
    ...
    def wait_for_condition(self, condition, timeout=None, error=None):
        ...
        self.base("", f"Condition: {condition} was met under {message}", f"{condition}", condition, timeout, error)

    @keyword
    def wait_until_location_is(self, expected, timeout=None, message=None):
        message = self._set_message(timeout)
        self.base("", f"Location was {expected} under {message}", f"{expected}", expected, timeout, message)

    @keyword
    def wait_until_location_is_not(self, location, timeout=None, message=None):
        message = self._set_message(timeout)
        self.base("", f"Location was not {location} under {message}", f"{location}", location, timeout, message)

    @keyword
    def wait_until_location_contains(self, expected, timeout=None, message=None):
        message = self._set_message(timeout)
        self.base("", f"Location contained {expected} under {message}", f"{expected}", expected, timeout, message)

    @keyword
    def wait_until_location_does_not_contain(self, location, timeout=None, message=None):
        message = self._set_message(timeout)
        self.base("", f"Location does not contain {location} under {message}", f"{location}", location, timeout, message)

    @keyword
    def wait_until_page_contains(self, text, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base("", f"Page contained {text} under {message}", f" {text}", text, timeout, error)

    @keyword
    def wait_until_page_does_not_contain(self, text, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base("", f"Page does not contain {text} under {message}", f" {text}", text, timeout, error)

    @keyword
    def wait_until_page_contains_element(self, locator, timeout=None, error=None, limit=None):
        message = self._set_message(timeout)
        self.base(locator, f"Page contained {locator} under {message}", f" {locator}", timeout, error, limit)

    @keyword
    def wait_until_page_does_not_contain_element(self, locator, timeout=None, error=None, limit=None):
        message = self._set_message(timeout)
        self.base(locator, f"Page does not contain {locator} under {message}", f" {locator}", timeout, error, limit)

    @keyword
    def wait_until_element_is_visible(self, locator, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base(locator, f"Element {locator} was visible under {message}", f" {locator}", timeout, error)

    @keyword
    def wait_until_element_is_not_visible(self, locator, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base(locator, f"Element {locator} was not visible under {message}", f" {locator}", timeout, error)

    @keyword
    def wait_until_element_is_enabled(self, locator, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base(locator, f"Element {locator} was enabled under {message}", f" {locator}", timeout, error)

    @keyword
    def wait_until_element_contains(self, locator, text, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base(locator, f"Element {locator} contained {text} under {message}", f" {text}", text, timeout, error)

    @keyword
    def wait_until_element_does_not_contain(self, locator, text, timeout=None, error=None):
        message = self._set_message(timeout)
        self.base(locator, f"Element {locator} does not contain {text} under {message}", f" {text}", text, timeout, error)

    def _set_message(self, timeout):
        return "set default timeout" if timeout is None else f"{timeout} seconds"
